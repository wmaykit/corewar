...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/08 14:37:10 by wmaykit           #+#    #+#             */
/*   Updated: 2020/03/07 14:05:29 by wmaykit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

int		operation_ldi(t_carriage *m_carrige, t_code *i_code, t_corewar *m_cor)
{
	int		content_arg[3];
	...
	if (i_code->arg_type[flag_type][arg1] & T_REG)
		...
		memory_read(m_cor->memory,
				memory_shift(m_carrige->address, i_code->args[arg1] % IDX_MOD),
					&content_arg[0], REG_SIZE);
	...
	memory_read(m_cor->memory, memory_shift(m_carrige->address,
				(content_arg[0] + content_arg[1]) % IDX_MOD),
