...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/07 16:11:35 by wmaykit           #+#    #+#             */
/*   Updated: 2020/03/07 14:02:51 by wmaykit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

int		operation_and(t_carriage *m_carrige, t_code *i_code, t_corewar *m_cor)
{
	int		data[3];
	...
	m_carrige->address + (i_code->args[arg1] % IDX_MOD), &data[0], REG_SIZE);
	if (i_code->arg_type[flag_type][arg2] & T_REG)
		...
		memory_read(m_cor->memory,
	m_carrige->address + (i_code->args[arg2] % IDX_MOD), &data[1], REG_SIZE);
